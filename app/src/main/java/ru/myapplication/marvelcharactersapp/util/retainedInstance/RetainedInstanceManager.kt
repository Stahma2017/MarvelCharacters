...
class RetainedInstanceManager<T> {
    ...
    fun init(app: Application) = app.registerActivityLifecycleCallbacks(
            ActivityLifecycleHelper(
                    onActivityDestroyed = { activity ->
                        ...
                        Log.d(RetainedInstanceManager::class.java.simpleName, "Instance for activity with key=$uuid removing.")
                        store.remove(uuid)

                    },
                    onFragmentDestroyed = { fragment ->
                        val uuid = (fragment as? IdProvider)?.getUuid() ?: return@ActivityLifecycleHelper
                        Log.d(RetainedInstanceManager::class.java.simpleName, "Instance for fragment with key=$uuid removing.")
                        store.remove(uuid)
                    }
            )
    )
    ...
    fun bindKodein(owner: IHasRetainedInstance<T>) : T {
        ...
        return when {
            store.isExist(key) -> {
                Log.d(RetainedInstanceManager::class.java.simpleName,"Instance for ${if (owner is Fragment) "fragment" else "activity"} with key=${key} getting from cache.")
                store.get(key)
            }
            else -> {
                Log.d(RetainedInstanceManager::class.java.simpleName,"Instance for ${if (owner is Fragment) "fragment" else "activity"} with key=${key} creating.")
